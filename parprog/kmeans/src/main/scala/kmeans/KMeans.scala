...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val rslt1 = points.groupBy(findClosest(_, means))
    // add any means not a K
    rslt1 ++ means.filterNot(rslt1.contains(_)).map(a => a -> IndexedSeq()).toMap
  }
  ...
  // IN output of classified, and the sequence of previous means.
  ...
  // true iff the square distance between the old and the new mean is less than or equal to eta, for all means.
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    var rslt = true
    var i = 1
    while (i < oldMeans.length && rslt) {
  ...
  //def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
  //def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
  // kMeans implements classify; update; converged
  @tailrec
