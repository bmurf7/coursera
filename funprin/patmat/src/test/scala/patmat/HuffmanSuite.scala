...
class HuffmanSuite extends FunSuite {
  ...
  test("singleton") {
    ...
    assert(singleton(makeOrderedLeafList(List(('t', 2), ('e', 1), ('x', 3)))) == false)
  }

  test("combine of some leaf list") {
    val leaflist = List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 4))
    assert(combine(leaflist) === List(Fork(Leaf('e',1),Leaf('t',2),List('e', 't'),3), Leaf('x',4)))
    val oneElement = List(Leaf('e', 1))
    assert(combine(List()) === List())
    assert(combine(oneElement) === oneElement)
    val twoElement = List(Leaf('e', 1),Leaf('a',2))
    assert(combine(twoElement) === List(Fork(Leaf('e', 1),Leaf('a',2),List('e','a'),3)))
  }
  ...
  test("encode french secret") {
    ...
    val txt = "huffmanestcool"
    val coded = encode(frenchCode)(string2Chars(txt))
  ...
  test("decode a very short text") {
    new TestTrees {
      ...
      assert(decode(t1,List(1,0,1)) === "bab".toList)
    }
  }
  ...
  test("convert CodeTree to CodeTable") {
    new TestTrees {
      val tbl1 = convert(t1)
      assert(tbl1 === List(('a',List[Bit](0)), ('b',List[Bit](1))))
      val tbl2 = convert(t2)
      assert(tbl2 === List( ('a',List[Bit](0,0)), ('b',List[Bit](0,1)), ('d',List[Bit](1)) ) )
    }
  }
  test("codeBits") {
    new TestTrees {
      val tbl = convert(t2)
      assert( codeBits(tbl)('a') === List[Bit](0,0) )
      assert( codeBits(tbl)('b') === List[Bit](0,1) )
      assert( codeBits(tbl)('d') === List[Bit](1) )
