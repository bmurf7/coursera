...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    val a = postings.filter(_.postingType == 2).filter(_.parentId.isDefined).map(p => (p.parentId.get,p))
    // join results in RDD[(QID, (Question, Answer))]
    // groupByKey results in RDD[(QID, Iterable[(Question, Answer)])] w/ distinct QID
    q.join(a).groupByKey()
  }


  /** Compute the maximum score for each posting */
  //  IN: RDD[(QID, Iterable[(Question, Answer)])]
  // OUT: RDD[(Question, HighScore)]
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    // 
    //  vectors RDD[(lang_index,max_score)]
    //  means   Array[(x, y)]
  ...
  //
  //  IN: means   Array[(x, y)]
  //  IN: vectors RDD[(lang_index,max_score)]
  // OUT: Array[(langLabel, langPercent, clusterSize, medianScore)]
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    val closest = vectors.map(p => (findClosest(p, means), p))  // RDD[(means_index, (lang_index, max_score))]
    val closestGrouped = closest.groupByKey()                   // RDD[(means_index, Iterable[(lang_index, max_score)])]
